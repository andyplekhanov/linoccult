...
namespace CGLA {
	...
	Vec3f::Vec3f(const Quaternion& q):
		ArithVec<double,Vec3f,3>(q.qv) {}

	Vec3f::Vec3f(const Vec3d& v):
		ArithVec<double,Vec3f,3>(v[0], v[1], v[2]) {}

	void Vec3f::get_spherical( double &theta, double &phi, double &rlen ) const
	{  
		rlen = length();
	...
	void Vec3f::set_spherical( double theta, double phi, double rlen )
	{
	...
	void orthogonal(const Vec3f& _a, Vec3f& b, Vec3f& c)
	{
		...
		double max_sqval=sqr(a[0]);
		int mi=0;
		for(int i=1;i<3;i++)
			{
				double sqval = sqr(a[i]);
				if(max_sqval<sqval)
					{
						max_sqval = sqval;
						mi = i;
