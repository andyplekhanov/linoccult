...
APSDE :: APSDE ( const APSAbsIntegFunction * afunc ) : APSAbsInteg( afunc )
{
  unsigned int Neqn = afunc->GetNeqn();
...
APSDE :: ~APSDE( void )
{
  ...
  delete[] yp;
  delete[] ypout;
  for (int i=0; i<=GetIntegFunction()->GetNeqn(); i++) delete[] phi[i];
    delete[] phi;
};
...
// Interpolation
//
void APSDE :: Intrp( const double x, const double y[], 
                     const double xout, double yout[], double ypout[] )
{
  //
  // Variables
  //
  ...
  for (l=1; l<=GetIntegFunction()->GetNeqn(); l++)  yout[l] = y[l] + hi*yout[l]; 
};
